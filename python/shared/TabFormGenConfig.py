...
class TabGenConfig(QTabWidget):
    ...
    def sendFormData(self):
        ...
        config_dict["doAuto"] = self.doauto_checkbox.isChecked()
        config_dict["doAutoWait"] = self.doautowait_checkbox.isChecked()
        config_dict["stopondata"] = self.stop_on_data_checkbox.isChecked()
        config_dict["noCompute"] = self.no_compute_checkbox.isChecked()
        config_dict["debugLevel"] = self.debug_level.value()
        config_dict["reportCompFramesLessThan"] = self.report_compute_frames.text()
        config_dict["reportGraphFramesLessThan"] = self.report_graphics_frames.text()
        # Add Section 3 Details
        config_dict["phys_device"] = self.gpu_model.text()
        config_dict["frame_delay"] = self.frame_delay.text()
        config_dict["end_frame"] = self.end_frame.text()
        config_dict["dt"] = self.dt.text()
        config_dict["compileShaders"] = self.compile_shaders_checkbox.isChecked()
        config_dict["frag_kernParticle"] = self.frag_kernel.text()
        config_dict["vert_kernParticle"] = self.vert_kernel.text()
        config_dict["comp_kernParticle"] = self.comp_kernel.text()
        # Add section 4 details
        config_dict["printExtension"] = self.rep_ext_checkbox.isChecked()
        config_dict["printDevLimtits"] = self.rep_lim_checkbox.isChecked()
        config_dict["enableValidationLayers"] = self.val_layers_checkbox.isChecked()
        config_dict["device_extensions"] = self.get_string_list(self.dev_ext_list_widget)
        config_dict["instance_extensions"] = self.get_string_list(self.ins_ext_list_widget)
        config_dict["validation_layers"] = self.get_string_list(self.val_lay_list_widget)
